...
def ssh_shutdown(hostname, username, password=None, key_filename=None):
...
def shutdown_device(device_id: str):
    ...
    # Parametri SSH (dovrebbero essere configurati per ogni dispositivo)
    # Per semplicità, usiamo valori di default
    ssh_username = request.json.get('username', 'admin')
    ssh_password = request.json.get('password', '')
    ssh_key = request.json.get('key_file', None)
